...
int main(int argc, char **argv) {
  ...
  std::vector<std::shared_ptr<xacc::Accelerator>>accelerators;
  std::vector<std::string> additional_accelerators{"tnqvm","tnqvm", "aer"};
  for (auto qpu : additional_accelerators) {
      auto accelerator = xacc::getAccelerator(qpu);
      accelerator->initialize({{"tnqvm-visitor", "exatn"}});
      xacc::info("Initialized accelertor: " + qpu);
  ...
  auto accelerator = xacc::getAcceleratorDecorator("hpc-het-virtualization", accelerators.front(),
                                              {{"shared-qpus", std::make_shared<std::vector<std::shared_ptr<xacc::Accelerator>>>(accelerators)}});
